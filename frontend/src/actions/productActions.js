...
export const listProducts = ({
  ...
  try {
    ...
    const { data } = await Axios.get(
      `$(API_URL)/api/products?pageNumber=${pageNumber}&seller=${seller}&name=${name}&category=${category}&min=${min}&max=${max}&rating=${rating}&order=${order}`
    );
...
export const updateProduct = (product) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.put(`$(API_URL)/api/products/${product._id}`, product, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
